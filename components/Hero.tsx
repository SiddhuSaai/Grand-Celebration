...
export default function Hero() {
  ...
  return (
    ...
    >
      <Image
        src={clientImages.heroMobile}
        alt={`${clientProfile.name} floral stage decor`}
        fill
        sizes="100vw"
        className="object-cover object-[48%_center] sm:hidden"
      />
      <Image
        src={clientImages.heroDesktop}
        alt={`${clientProfile.name} decorated stage backdrop`}
        fill
        priority
        sizes="100vw"
        className="hidden object-cover object-[50%_center] sm:block"
      />
      ...
      >
        ...
        >
          Balloon Decorators & Event Organisers in {clientProfile.city}
        </motion.p>
        <motion.h1
          variants={item}
          className="mx-auto max-w-5xl font-serif text-4xl leading-[0.98] text-white min-[380px]:text-5xl sm:text-6xl sm:leading-[0.95] lg:text-[76px]"
        >
        ...
        >
          Wedding stage decor, birthday decoration, balloon styling, lighting, and event
          management from {clientProfile.location}, starting at {clientProfile.startingPrice}.
        </motion.p>
        ...
        <motion.div
          variants={item}
          className="mx-auto mt-12 hidden max-w-3xl grid-cols-3 border-y border-white/15 bg-black/20 py-4 text-white/80 sm:grid"
        >
          {[
            [clientProfile.yearsInBusiness, 'Years Service'],
            [`${clientProfile.rating}★`, 'Customer Rating'],
